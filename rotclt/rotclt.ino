...
constexpr uint8_t LED_MOTOR_ENABLE = 0; // D3, active LOW
...
MotorDriver motorDriver(appState, AZ_STEP, AZ_DIR, EL_STEP, EL_DIR, ENABLE_PIN,
                        true);
LedIndicator led(LED_PIN, LED_MOTOR_ENABLE, true);
...
void loop() {
  ...
  // Enable motors only when a rotctl client is connecting or when is
  // disconnectiong to prevent unwanted movement when not in use
  ...
  if ((now - lastPositionSaveMs >= 5000 &&
       (deltaAz >= 0.1f || deltaEl >= 0.1f)) ||
  ...
  led.setMode(appState.clientConnected ? LedMode::RotctlConnected
                                       : LedMode::Idle);
  led.setMotorEnable(appState.enable);
  led.tick();
}
