...
function invertAz(az) {
  return az;//((360 - az) % 360);
}

function setDishRotation(group, az, el) {
  const invAz = invertAz(az);
  group.rotation.order = 'YXZ';
  group.rotation.y = -THREE.MathUtils.degToRad(invAz);
  group.rotation.x = -THREE.MathUtils.degToRad(el);
}

function getDishDir(az, el) {
  const invAz = invertAz(az);
  const a = -THREE.MathUtils.degToRad(invAz);
...
function updateUI() {
  ...
  document.getElementById('curAz').textContent = invertAz(D.currentAz).toFixed(1) + '\xb0';
  document.getElementById('curEl').textContent = D.currentEl.toFixed(1) + '\xb0';
  document.getElementById('tarAz').textContent = invertAz(D.targetAz).toFixed(1) + '\xb0';
...
updateUI = function() {
  ...
  if (mqAz) mqAz.textContent = invertAz(D.currentAz).toFixed(1) + '\xb0';
  if (mqEl) mqEl.textContent = D.currentEl.toFixed(1) + '\xb0';
  if (mqTAz) mqTAz.textContent = invertAz(D.targetAz).toFixed(1) + '\xb0';
